...
describe('services/data/disable-non-ticketed-expenses-for-claim', () => {
  ...
  beforeEach(() => {
    return eligiblityHelper.insertEligibilityClaim(REFERENCE).then(function (ids) {
      ...
      return insertExpense(REFERENCE, eligibilityId, claimId, ticketedExpense).then(function (expenseIds) {
        const documentForTicketedExpense = documentHelper.buildExpenseDoc(claimId, expenseIds[0].ClaimExpenseId)
        return insertClaimDocument(REFERENCE, eligibilityId, claimId, documentForTicketedExpense).then(() => {
          return insertExpense(REFERENCE, eligibilityId, claimId, nonticketedExpense).then(function (expenseIds) {
  ...
  it('should disable previous expenses for non-ticketed expense types', () => {
    return disableNonTicketedExpensesForClaim(REFERENCE, eligibilityId, claimId, NON_TICKETED_EXPENSE_TYPE).then(
      () => {
        return expenseHelper.getAll(claimId).then(function (expenses) {
  ...
  it('should disable documents supporting previous expenses for non-ticketed expense types', () => {
    return disableNonTicketedExpensesForClaim(REFERENCE, eligibilityId, claimId, NON_TICKETED_EXPENSE_TYPE).then(
      () => {
        return expenseHelper.getAll(claimId).then(function (expenses) {
          expenses.forEach(expense => {
            if (expense.ExpenseType === NON_TICKETED_EXPENSE_TYPE) {
              return documentHelper.getAllForExpense(expense.ClaimExpenseId).then(function (documents) {
  ...
  it('should not disable documents if expenses are not disabled', () => {
    return expenseHelper.getAll(claimId).then(function (expenses) {
      expenses.forEach(expense => {
        if (expense.ExpenseType === NON_TICKETED_EXPENSE_TYPE) {
          return documentHelper.getAllForExpense(expense.ClaimExpenseId).then(function (documents) {
  ...
  it('should not disable previous expenses for ticketed expense types', () => {
    return disableNonTicketedExpensesForClaim(REFERENCE, eligibilityId, claimId, TICKETED_EXPENSE_TYPE).then(
      () => {
        return expenseHelper.getAll(claimId).then(function (expenses) {
          expenses.forEach(expense => {
  ...
  it('should not disable documents supporting previous expenses for ticketed expense types', () => {
    return disableNonTicketedExpensesForClaim(REFERENCE, eligibilityId, claimId, TICKETED_EXPENSE_TYPE).then(
      () => {
        return expenseHelper.getAll(claimId).then(function (expenses) {
          expenses.forEach(expense => {
