...
describe('routes/apply/eligibility/claim/accommodation-details', () => {
  const COOKIES = [
  ...
  beforeEach(() => {
    ...
    jest.mock('../../../../../../app/services/domain/expenses/accommodation-expense', () => mockAccommodationExpense)
    jest.mock('../../../../../../app/services/data/get-is-advance-claim', () => mockGetIsAdvanceClaim)

    const route = require('../../../../../../app/routes/apply/eligibility/claim/accommodation-details')
  ...
  describe(`POST ${ROUTE}`, () => {
    ...
    it('should respond with a 302 if domain object is built and then persisted successfully', () => {
      mockAccommodationExpense.mockReturnValue(ACCOMMODATION_EXPENSE)
      mockInsertExpense.mockResolvedValue()
      return supertest(app)
        .post(ROUTE)
        .set('Cookie', COOKIES)
        .expect(() => {
          expect(mockAccommodationExpense).toHaveBeenCalledTimes(1)
    ...
    it('should respond with a 400 if domain object validation fails.', () => {
      mockAccommodationExpense.mockImplementation(() => {
