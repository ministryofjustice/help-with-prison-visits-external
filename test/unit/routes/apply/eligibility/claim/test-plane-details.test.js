...
describe('routes/apply/eligibility/claim/plane-details', () => {
  ...
  const mockPlaneExpense = jest.fn()
  ...
  beforeEach(() => {
    ...
    jest.mock('../../../../../../app/services/domain/expenses/plane-expense', () => mockPlaneExpense)
    jest.mock('../../../../../../app/services/data/get-expense-owner-data', () => mockGetExpenseOwnerData)
    jest.mock('../../../../../../app/services/data/get-is-advance-claim', () => mockGetIsAdvanceClaim)

    const route = require('../../../../../../app/routes/apply/eligibility/claim/plane-details')
  ...
  describe(`GET ${ROUTE}`, () => {
    ...
    it('should call the function to get expense owner data', () => {
      mockGetExpenseOwnerData.mockResolvedValue({})
      return supertest(app)
        .get(ROUTE)
        .set('Cookie', COOKIES)
  ...
  describe(`POST ${ROUTE}`, () => {
    ...
    it('should respond with a 302 if domain object is built and then persisted successfully', () => {
      mockPlaneExpense.mockReturnValue(PLANE_EXPENSE)
      mockInsertExpense.mockResolvedValue()
      return supertest(app)
        .post(ROUTE)
        .set('Cookie', COOKIES)
        .expect(() => {
          expect(mockPlaneExpense).toHaveBeenCalledTimes(1)
    ...
    it('should respond with a 400 if domain object validation fails.', () => {
      mockGetExpenseOwnerData.mockResolvedValue({})
      mockPlaneExpense.mockImplementation(() => {
