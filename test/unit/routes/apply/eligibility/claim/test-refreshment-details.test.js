...
describe('routes/apply/eligibility/claim/light-refreshment-details', () => {
  const COOKIES = [
  ...
  const ROUTE = '/apply/eligibility/claim/refreshment'
  ...
  beforeEach(() => {
    ...
    jest.mock('../../../../../../app/services/domain/expenses/refreshment-expense', () => mockRefreshmentExpense)
    jest.mock('../../../../../../app/services/data/get-is-advance-claim', () => mockGetIsAdvanceClaim)

    const route = require('../../../../../../app/routes/apply/eligibility/claim/light-refreshment-details')
  ...
  describe(`POST ${ROUTE}`, () => {
    ...
    it('should respond with a 302 if domain object is built and then persisted successfully', () => {
      mockRefreshmentExpense.mockReturnValue(REFRESHMENT_EXPENSE)
      mockInsertExpense.mockResolvedValue()
      return supertest(app)
        .post(ROUTE)
        .set('Cookie', COOKIES)
        .expect(() => {
          expect(mockRefreshmentExpense).toHaveBeenCalledTimes(1)
    ...
    it('should respond with a 400 if domain object validation fails.', () => {
      mockRefreshmentExpense.mockImplementation(() => {
