...
describe('routes/apply/eligibility/claim/car-hire-details', function () {
  ...
  const CLAIMID = '1'
  const ROUTE = `/apply/first-time/eligibility/${ENCRYPTED_REFERENCEID}/claim/${CLAIMID}/hire`
  ...
  beforeEach(function () {
    ...
    var route = proxyquire('../../../../../../app/routes/apply/eligibility/claim/car-hire-details', {
    ...
    it('should respond with a 302 if domain object is built and then persisted successfully', function () {
      ...
      return supertest(app)
        ...
        .expect(function () {
          ...
          sinon.assert.calledWith(insertExpenseStub, REFERENCE, ELIGIBILITYID, CLAIMID, HIRE_EXPENSE)
        })
        .expect(302)
    })
