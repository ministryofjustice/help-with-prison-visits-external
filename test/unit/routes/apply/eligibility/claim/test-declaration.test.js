...
describe('routes/apply/eligibility/claim/declaration', () => {
  const COOKIES = [
  ...
  describe(`POST ${ROUTE}`, () => {
    ...
    it('should respond with a 302 and call submit claim and put past in route', () => {
    ...
    it('should use assisted digital cookie value', () => {
    ...
    it('should just go to redirect if checkStatusForFinishingClaim returns false in case of double submission', () => {
      mockDeclaration.mockReturnValue()
      mockGetIsAdvanceClaim.mockResolvedValue()
    ...
    it('should respond with a 400 if validation fails', () => {
      mockDeclaration.mockImplementation(() => {
        throw new ValidationError({ firstName: {} })
      })
      return supertest(app).post(ROUTE).set('Cookie', COOKIES).expect(400)
    })

    it('should respond with a 500 if promise rejects on submitting claim.', () => {
      mockDeclaration.mockReturnValue()
