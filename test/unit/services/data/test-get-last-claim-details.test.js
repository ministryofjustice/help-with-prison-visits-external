...
const EXPENSES = [
  { ClaimExpenseId: 2, ExpenseType: 'bus', Cost: '20' },
  { ClaimExpenseId: 3, ExpenseType: 'taxi', Cost: '15' },
  { ClaimExpenseId: 4, ExpenseType: 'train', Cost: '10' },
  { ClaimExpenseId: 5, ExpenseType: 'train', Cost: '5' },
]
...
describe('services/data/get-last-claim-details', () => {
  beforeEach(() => {
  ...
  it('should call to get last claim children and last claim expenses', () => {
    mockGetLastClaimForReference.mockResolvedValue(CLAIMID)
    return getLastClaimDetails(REFERENCE, ELIGIBILITYID, false, false).then(result => {
  ...
  it('should mask child last name and escort last name if mask is true', () => {
    mockGetLastClaimForReference.mockResolvedValue(CLAIMID)
    return getLastClaimDetails(REFERENCE, ELIGIBILITYID, true, false).then(result => {
      expect(mockGetLastClaimForReference).toHaveBeenCalledWith(REFERENCE, ELIGIBILITYID)
      expect(mockGetClaimChildrenByIdOrLastApproved).toHaveBeenCalledWith(REFERENCE, ELIGIBILITYID, CLAIMID[0].ClaimId)
      expect(mockGetClaimExpenseByIdOrLastApproved).toHaveBeenCalledWith(REFERENCE, ELIGIBILITYID, CLAIMID[0].ClaimId)
      expect(mockGetClaimEscortByIdOrLastApproved).toHaveBeenCalledWith(REFERENCE, ELIGIBILITYID, CLAIMID[0].ClaimId)
  ...
  it('should get last claim and remove train expenses if last claim type is not the same as this claim type', () => {
    mockGetLastClaimForReference.mockResolvedValue(CLAIMID)
    return getLastClaimDetails(REFERENCE, ELIGIBILITYID, false, true).then(result => {
      expect(result.expenses.filter(expense => expense.ExpenseType === 'train').length).toBe(0)
      expect(result.expenses.length).toBe(2)
    })
  })

  it('should get last claim and keep train expenses if last claim type is the same as this claim type', () => {
    mockGetLastClaimForReference.mockResolvedValue(CLAIMID)
    return getLastClaimDetails(REFERENCE, ELIGIBILITYID, false, false).then(result => {
  ...
  it('should get last claim and reset train expenses to zero if last claim type is the same as this claim type', () => {
