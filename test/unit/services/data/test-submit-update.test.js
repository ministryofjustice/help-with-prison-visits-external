...
const BANK_DETAILS = {
  ...
  nameOnAccount: 'MR JOSEPH BLOGGS',
  rollNumber: '',
  required: true,
}
const TEST_BANK_ACCOUNT_DETAILS = new BankAccountDetails(
  BANK_DETAILS.accountNumber,
  BANK_DETAILS.sortCode,
  BANK_DETAILS.nameOnAccount,
  BANK_DETAILS.rollNumber,
  'yes',
)
const NO_BANK_DETAILS = { required: false }

describe('services/data/submit-update', () => {
  let submitUpdate
  ...
  it('should insert REQUEST_INFORMATION_RESPONSE task', () => {
    mockInsertTask.mockResolvedValue()
    return submitUpdate(REFERENCE, ELIGIBILITYID, CLAIMID, '', NO_BANK_DETAILS).then(() => {
  ...
  it('should insert bank details if they have been updated', () => {
    mockInsertTask.mockResolvedValue()
    mockInsertBankDetails.mockResolvedValue()
    return submitUpdate(REFERENCE, ELIGIBILITYID, CLAIMID, '', BANK_DETAILS).then(() => {
        expect(mockInsertTask).toHaveBeenCalledWith(REFERENCE, ELIGIBILITYID, CLAIMID, tasksEnum.REQUEST_INFORMATION_RESPONSE, '', false)  //eslint-disable-line
