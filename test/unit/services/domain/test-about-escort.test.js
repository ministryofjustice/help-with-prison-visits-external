...
describe('services/domain/about-escort', () => {
  ...
  it('should construct a domain object given valid input', () => {
    const escort = new AboutEscort(VALID_FIRST_NAME, VALID_LAST_NAME, VALID_DAY, VALID_MONTH, VALID_YEAR)
  ...
  it('should throw an error if passed invalid data', () => {
    expect(() => {
      new AboutEscort(INVALID_DAY, INVALID_DAY, VALID_MONTH, VALID_YEAR).isValid()
    }).toThrow()
  })
  ...
  it('should strip illegal characters from otherwise valid input', () => {
    ...
    const escort = new AboutEscort(INVALID_CHARS_FIRST_NAME, VALID_LAST_NAME, VALID_DAY, VALID_MONTH, VALID_YEAR)
