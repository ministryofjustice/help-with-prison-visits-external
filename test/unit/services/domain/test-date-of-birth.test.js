...
describe('services/domain/date-of-birth', () => {
  const VALID_DAY = '01'
  ...
  it('should construct a domain object given valid input', () => {
    dateOfBirth = new DateOfBirth(VALID_DAY, VALID_MONTH, VALID_PAST_YEAR)
  ...
  it('should throw ValidationError if a future date was given', () => {
    expect(() => {
      new DateOfBirth(VALID_DAY, VALID_MONTH, VALID_FUTURE_YEAR)
    }).toThrow()
  })

  it('should throw ValidationError if date is under 16 years', () => {
    expect(() => {
      new DateOfBirth(dateFormatter.now().date(), dateFormatter.now().month() + 1, dateFormatter.now().year())
    }).toThrow()
  })

  it('should throw ValidationError if given invalid day, month, or year', () => {
    expect(() => {
      new DateOfBirth(INVALID_DAY, INVALID_MONTH, INVALID_YEAR)
    }).toThrow()
  })
})
