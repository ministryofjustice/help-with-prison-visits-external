...
describe('services/domain/expenses/bus-expense', () => {
  const VALID_COST = '20'
  ...
  it('should construct a domain object given valid input', () => {
    const expense = new BusExpense(VALID_COST, VALID_FROM, VALID_TO, VALID_IS_RETURN, VALID_TICKET_OWNER)
  ...
  it('should throw an error if passed invalid data', () => {
    expect(() => {
      new BusExpense(INVALID_COST, VALID_FROM, VALID_TO, VALID_IS_RETURN, VALID_TICKET_OWNER).isValid()
    }).toThrow()
  })
})
