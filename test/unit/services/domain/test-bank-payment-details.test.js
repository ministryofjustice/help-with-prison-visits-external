...
describe('services/domain/payment-details', () => {
  ...
  it('should construct a domain object given a sort code with hyphens', () => {
  ...
  it('should construct a domain object given a sort code with hyphens and spaces', () => {
    const sortCodeWithHyphensAndSpaces = '12 - 12 - 12'
    const processedSortCodeWithHyphensAndSpaces = '121212'
    bankAccountDetails = new BankAccountDetails(
      VALID_ACCOUNT_NUMBER,
      sortCodeWithHyphensAndSpaces,
      VALID_NAME_ON_ACCOUNT,
      VALID_ROLL_NUMBER,
    )
    expect(bankAccountDetails.accountNumber).toBe(PROCESSED_ACCOUNT_NUMBER)
  ...
  it('should throw a ValidationError if given letters', () => {
    expect(() => {
      new BankAccountDetails('asdf', 'asdf', VALID_NAME_ON_ACCOUNT, VALID_ROLL_NUMBER)
    }).toThrow()
  })

  it('should throw a ValidationError if given invalid length inputs', () => {
    expect(() => {
      new BankAccountDetails('123456789', '123', VALID_NAME_ON_ACCOUNT, VALID_ROLL_NUMBER)
    }).toThrow()
  })

  it('should throw a ValidationError if given invalid name on account length', () => {
    expect(() => {
      new BankAccountDetails(
        '12345678',
        '123456',
        'ztEHgAjGtoMIWQUmeFQRlDmrklPGpSPiLUYpenFuyeVoaAbxENBeWXdhkEmujscmMSWJreKJrzawFgEpUWxrHeQRnMVdbOVejsOcw',
        '123456789012345678',
      )
    }).toThrow()
  })

  it('should throw a ValidationError if given invalid roll number length', () => {
    expect(() => {
      new BankAccountDetails('12345678', '123456', VALID_NAME_ON_ACCOUNT, '1234567890123456789')
    }).toThrow()
  })

  it('should throw a ValidationError if roll number contains invalid characters', () => {
    expect(() => {
