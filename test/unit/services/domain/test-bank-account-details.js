...
describe('services/domain/bank-account-details', function () {
  ...
  it('should construct a domain object given valid input', function (done) {
    bankAccountDetails = new BankAccountDetails(VALID_ACCOUNT_NUMBER, VALID_SORT_CODE, VALID_T_AND_CS)
    ...
    expect(bankAccountDetails.tAndCs).to.equal(VALID_T_AND_CS)
    done()
  })
  ...
  it('should return isNumber errors given letters', function (done) {
    try {
      bankAccountDetails = new BankAccountDetails('asdf', 'asdf', VALID_T_AND_CS)
    } catch (e) {
      expect(e).to.be.instanceof(ValidationError)
  ...
  it('should return isLength errors given invalid length', function (done) {
    try {
      bankAccountDetails = new BankAccountDetails('123456789', '123', VALID_T_AND_CS)
    } catch (e) {
      expect(e).to.be.instanceof(ValidationError)
