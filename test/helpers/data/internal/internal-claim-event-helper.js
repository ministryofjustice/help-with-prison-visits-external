...
module.exports.build = (dateAdded, caseworker, additionalData, note, isInternal) => {
  return {
...
module.exports.get = claimId => {
  ...
  return db.first().from('IntSchema.ClaimEvent').where('ClaimId', claimId)
}

module.exports.delete = claimId => {
  const db = getDatabaseConnector()

  return db('IntSchema.ClaimEvent').where('ClaimId', claimId).del()
}
