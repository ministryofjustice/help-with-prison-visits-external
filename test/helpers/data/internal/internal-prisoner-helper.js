...
module.exports.insert = (reference, eligibilityId) => {
  ...
  return db('IntSchema.Prisoner').insert({
...
module.exports.get = reference => {
  ...
  return db.first().from('IntSchema.Prisoner').where('Reference', reference)
}

module.exports.delete = reference => {
  const db = getDatabaseConnector()

  return db('IntSchema.Prisoner').where('Reference', reference).del()
}
