...
module.exports.insert = (reference, eligibilityId) => {
  ...
  return db('ExtSchema.Prisoner').insert({
...
module.exports.get = reference => {
  ...
  return db.first().from('ExtSchema.Prisoner').where('Reference', reference)
}

module.exports.delete = reference => {
  const db = getDatabaseConnector()

  return db('ExtSchema.Prisoner').where('Reference', reference).del()
}
