...
module.exports = router => {
  router.get('/apply/eligibility/claim/expenses', (req, res, next) => {
    ...
    getClaimSummary(req.session.claimId, req.session.claimType).then(claimDetails => {
  ...
  router.post('/apply/eligibility/claim/expenses', (req, res, next) => {
    UrlPathValidator(req.params)
    const isValidSession = SessionHandler.validateSession(req.session, req.url)
    ...
    try {
      ...
      if (typeof req.body?.expenses === 'object') {
        expenses = Object.values(req.body.expenses)
      }

      new Expenses(expenses) // eslint-disable-line no-new
