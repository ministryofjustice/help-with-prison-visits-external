...
module.exports = router => {
  router.get('/apply/eligibility/claim/taxi', (req, res, next) => {
  ...
  router.post('/apply/eligibility/claim/taxi', (req, res, next) => {
    UrlPathValidator(req.params)
    const isValidSession = SessionHandler.validateSession(req.session, req.url)
    ...
    try {
      const expense = new TaxiExpense(req.body?.cost, req.body?.from, req.body?.to)
